...
RSpec.describe "PropertyTenants", type: :request do
  ...
  describe "GET /settings/properties/:property_setting_id/property_tenants/new" do
    it "renders the new template" do
      get new_property_setting_property_tenant_path(property)
  ...
  describe "POST /settings/properties/:property_setting_id/property_tenants" do
    context "with valid parameters" do
      it "creates a new property tenant" do
        # Use tenant2 since tenant_one is already assigned via fixture
        expect {
          post property_setting_property_tenants_path(property), params: {property_tenant: {tenant_id: tenant2.id, rent_amount: 1000}}
    ...
    context "with invalid parameters" do
      it "does not create a new property tenant without rent_amount" do
        expect {
          post property_setting_property_tenants_path(property), params: {property_tenant: {tenant_id: tenant.id}}
      ...
      it "does not create a duplicate property tenant" do
        # Fixture already creates property_tenant_one with property_one and tenant_one
        expect {
          post property_setting_property_tenants_path(property), params: {property_tenant: {tenant_id: tenant.id, rent_amount: 2000}}
        }.not_to change(PropertyTenant, :count)
      end
    end
  end

  describe "DELETE /settings/properties/:property_setting_id/property_tenants/:id" do
    let(:property_tenant) { property_tenants(:property_tenant_one) }

    it "destroys the property tenant" do
      expect {
        delete property_setting_property_tenant_path(property, property_tenant)
      }.to change(PropertyTenant, :count).by(-1)
    end
